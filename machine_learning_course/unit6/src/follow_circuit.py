...
def load_weights():
    ...
    weight_path = '/home/user/catkin_ws/src/machine_learning_course/weights_proj'
    W2 = np.load(weight_path + '/w2.npy')
    b2 = np.load(weight_path + '/b2.npy')
    W = np.load(weight_path + '/w.npy')
    b = np.load(weight_path + '/b.npy')
...
def test_prediction(XX, W2, b2, W, b):
...
def callback(msg):
    ...
    (roll_, pitch_, theta_) = euler_from_quaternion(
...
def take_action():
    ...
    """
    theta_norm = normalize([theta], {'actual': {
                           'lower': -3.1415, 'upper': 3.1415}, 'desired': {'lower': -1.3962, 'upper': 1.3962}})
    theta_norm = theta_norm[0]
    y1, x1 = pol2cart(radar_front, theta_norm)

    x1_norm = normalize(
        [x1], {'actual': {'lower': -2, 'upper': 2}, 'desired': {'lower': -1, 'upper': 1}})
    x1_norm = x1_norm[0]
